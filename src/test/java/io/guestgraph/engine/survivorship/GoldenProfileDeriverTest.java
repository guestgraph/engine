package io.guestgraph.engine.survivorship;
...
class GoldenProfileDeriverTest {
  ...
  void mostRecentNonNullWinsPerField() {
    SourceRecord older =
        record(
            Map.of("firstName", "Anna", "email", "old@example.com"),
  ...
  void nullAndBlankValuesNeverOverwriteOlderData() {
    ...
    SourceRecord newer =
        record(
            withNull, Instant.parse("2026-06-01T00:00:00Z"), Instant.parse("2026-06-01T00:00:00Z"));

    Map<String, Object> profile = deriver.derive(List.of(older, newer));

    assertThat(profile).containsEntry("firstName", "Anna").containsEntry("lastName", "Muster");
  }
  ...
  void recordTimestampFallsBackToReceivedAt() {
    ...
    SourceRecord early =
        record(
            Map.of("firstName", "Old"),
            Instant.parse("2026-03-01T00:00:00Z"),
            Instant.parse("2026-03-01T00:00:00Z"));
    SourceRecord late =
        record(Map.of("firstName", "New"), null, Instant.parse("2026-06-01T00:00:00Z"));
  ...
  void conflictingValuesAcrossThreeRecordsResolveByRecency() {
    SourceRecord first =
        record(
            Map.of("email", "one@example.com", "phone", "+41790000001"),
            Instant.parse("2026-01-01T00:00:00Z"),
            Instant.parse("2026-01-01T00:00:00Z"));
    SourceRecord second =
        record(
    ...
    SourceRecord third =
        record(
            Map.of("email", "three@example.com", "firstName", "Anna"),
            Instant.parse("2026-03-01T00:00:00Z"),
            Instant.parse("2026-03-01T00:00:00Z"));
  ...
  private SourceRecord record(
      Map<String, Object> extracted, Instant recordTimestamp, Instant receivedAt) {
