...
- [X] T003 [P] Copy the engine's guardrails: `config/pmd-ruleset.xml`, the Spotless configuration with google-java-format, and `src/test/java/io/guestgraph/connector/apaleo/architecture/PersistenceRulesTest.java` holding repositories to `@Query`-only methods and JPA to the `state` package
