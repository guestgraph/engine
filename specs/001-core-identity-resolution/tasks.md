...
- [X] T017 [US1] Implement the resolution orchestrator (built as `src/main/java/io/guestgraph/resolution/ResolutionEngine.java` behind the storage-agnostic `GraphPort`): find candidates by shared identifiers, apply strategy, execute create/attach/merge outcomes, write MergeEvents (CREATE/ATTACH/MERGE with evidence), rebuild guest identifiers, run inside `TenantLock` — until T013 is green
- [X] T018 [US1] Implement `GoldenProfileDeriver` pure function in `src/main/java/io/guestgraph/survivorship/GoldenProfileDeriver.java` until T014 is green; ResolutionEngine recomputes `guest.profile` on every link change
