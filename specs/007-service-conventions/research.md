...
| API document | an `api/` directory in the service root holds at least one OpenAPI file, `pom.xml` bundles it, and a class under `api` maps `/api-docs`; a file pinned to another repository by `api.json` equals that repository's file at the pinned commit |
...
**Decision**: The connector vendors `specs/005-apaleo-connector/contracts/connector-api.yaml`
from the engine repository into its own `api/connector-api.yaml`, pinned by `api.json` with the
engine commit, and serves it at `/api-docs` through the same kind of controller the engine has,
reading the bundled `api-contracts` classpath directory. The service check compares the copy with
the engine's file at the pinned commit. The engine keeps serving the union of
`specs/*/contracts/openapi.yaml`; the connector's contract is not among them by name, as slice 5
decided.

**Rationale**: The contract's owner is the engine repository, where the connector's specification
lives and is frozen; a copy held against the owner at a pinned commit is how the sites take the
design system, so nothing new is invented, and a stale copy fails a check rather than drifting.
...
of R4; `docs/er-schema.mmd` regenerated by the shared script with `er-drift` in its workflow; and a
